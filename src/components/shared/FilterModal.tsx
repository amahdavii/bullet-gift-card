...
const FilterModal = () => {
  ...
  // مدال فقط وقتی باز میشه که کویری modal=filter باشه
  const isOpen = searchParams.get("modal") === "filter";

  const handleSelect = (value: string) => {
    const params = new URLSearchParams(Array.from(searchParams.entries()));
    params.delete("modal"); // مدال بسته بشه
    ...
    <BottomSheetModal isOpen={isOpen} onClose={handleClose} height="95vh">
      <div className="flex flex-col h-full">
        ...
        {!!getLabel && (
          <div className="px-3">
            <span
              className="bg-[#F1F0F0] text-[0.75rem] inline-flex items-center px-2 py-1 rounded gap-[0.5rem] cursor-pointer"
              onClick={handleReset}
            >
              {getLabel}
              <CloseSVG />
        ...
        <div className="flex-1 overflow-y-auto mt-4 px-4">
          <ul>
            ...
            <li
              className="py-4 px-2 border-[#E3E1E2] border-t border-b cursor-pointer hover:bg-gray-100"
